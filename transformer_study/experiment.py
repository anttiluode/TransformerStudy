...
from .config import ExperimentConfig, gate0_config, smoke_config
...
def run_experiment(preset_name: str, output: str | Path) -> dict:
    ...
    novelty = analyze_novelty(
        test_bank, trained_competence=competence,
        delta_competent=delta_row["competent"], min_split_n=cfg.correct_split_min_n,
    )
    ...
    render_results(out, preset_name, cfg, history, behavior, separability, linear_maps, composition, novelty)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="TransformerStudy Gate 0")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--preset", choices=["smoke", "gate0"])
